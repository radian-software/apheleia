...
(defmacro apheleia--with-single-undo (content-buffer &rest body)
  "Within CONTENT-BUFFER, execute BODY as a single undo step."
  (declare (indent 1))
  (let ((marker (cl-gensym "marker")))
    `(let ((,marker (prepare-change-group ,content-buffer)))
       (unwind-protect
           (progn ,@body)
